...
def main():
    ...
    pygame.display.set_caption("CherryPacMan")

    labyrinth = Labyrinth("simple_map.txt", [0, 2], 2)
    hero = Hero("hero.jpg", (7, 7))
    enemy = Enemy("enemy.jpg", (7, 1))
    ...
    while running:
        ...
        if game.check_win():
            game_over = True
            show_message(screen, "ВЫ СБЕЖАЛИ ОТ ИГР!")
        if game.check_lose():
            game_over = True
            show_message(screen, "О НЕТ, ВЫ ПРОИГРАЛИ!")
        pygame.display.flip()
        clock.tick(FPS)
    pygame.display.quit()
